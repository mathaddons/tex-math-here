...
//Insert custom corrections here following the same format as below. Make sure that the codes and corrections are entered in the same order
customCodes = []
customCorrections = []
...
//Define checkAgainstList function
function checkAgainstList(symbolCode) {                                             //Searches for symbolCode in a list of corrections, returns a string of the corrected version (or null if a match is not found)
    var i;
...
document.addEventListener('keypress', function (e) {
    ...
    if (!backslashFlag) {                                                                                               //Listen for leading backslash
        if (charCode == 92) {
            backslashFlag = true;
            symbolCode = [];
            position = 0;
    ...
    else {                                                                                                              //Record after leading backslash
        if (charCode == 32 || charCode == 13) {                                                                         //Check at space or carriage return (want to include tab = 9, but does not work in keypress or keydown)
        ...
        else if (charCode == 8) {                                                                                       //Deal with backspace
            position--;
...
document.addEventListener('keydown', function (e) {                                                                     //Deal with keys that don't trigger keypress
    e = e || window.event;
    var charCode = typeof e.which == "number" ? e.which : e.keyCode;
    if (charCode == 37 || charCode == 38 || charCode == 39 || charCode == 40) {                                         //Break at arrow keys
        backslashFlag = false;
    }
});