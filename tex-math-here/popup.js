document.addEventListener('DOMContentLoaded', function () {
    ...
    //Sets values of the options before the user has entered their own selections (which would then be stored in localStorage)
    ...
    input.addEventListener('input', function () {
        //User's input is stored in the text box after they click away from the extension (assumes that localstorage is supported on the browser)
        localStorage.setItem("text", input.value);

        //Alert to make the user aware that certain text will be removed in the php file
        function checkInput(text){
                if (input.value.includes(text)){
                        //Alerted is set to "yes" if the alert has already been shown for the key value, otherwise it is set to null
                        //Session storage used so the value "yes" does not persist if the extension is closed and then reopened
                        let alerted = sessionStorage.getItem(text) || null;
                        //If the alert has not been shown yet (and therefore is set to null), the alert message will appear and the key's value is set to "yes"
                        if(alerted != "yes")
                        {
    ...
    // SENDS MESSAGE TO latex_transport.js, QUERYING IF DOUBLE CLICK EDIT HAS BEEN USED. IF YES, DATA IS SENT BACK AND STORED IN retrieved_latex
    Browser.runtime.sendMessage({ type: 'latex_retrieve' }, function(retrieved_latex) {
        ...
        //If the color is changed through the color picker menu the rgb values are updated accordingly. Reference for hexToRgb function: https://stackoverflow.com/questions/5623838/rgb-to-hex-and-hex-to-rgb
    ...
    form.addEventListener('submit', function (e) {
       ...
       let dataString = "dpi=" + DPI.value + "&latex=" + latex + "&red=" + red.value + "&green=" + green.value + "&blue=" + blue.value + "&font=" + font.value
               ...
               if (xhr.readyState == 4)
                {
                        ...
                        sel.removeAllRanges();                     // CLEARS THE SELECTION SO THAT NOTHING BUT WHAT IT SELECTS NEXT IS SELECTED WHEN THE COPY COMMAND IS EXECUTED
                        sel.addRange(r);
                        document.execCommand('Copy');
                        sel.removeAllRanges();                  // CLEARS THE SELECTION SO THAT THE IMAGE DOESN'T LOOK BLUE AT THE END (AESTHETIC)
                }
                ...
                //10 seconds in milliseconds, triggers xhr.status = 0 if it hasn't proccessed already or hasn't already arrived at that state
                xhr.timeout = 10000;  
